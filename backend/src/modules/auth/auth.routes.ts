...
const createSchema = z.object({ body: credentials.extend({ roleName: z.enum([ROLES.SYSTEM_ADMINISTRATOR, ROLES.VIEWER]) }), params: z.object({}), query: z.object({}) });

export const authRouter = Router();
authRouter.post("/login", validate(loginSchema), login);
authRouter.post("/signup", validate(signupSchema), register);
authRouter.post("/users", authenticate, authorize(ROLES.SYSTEM_ADMINISTRATOR), validate(createSchema), createByAdmin);
