...
export const register: RequestHandler = async (req, res, next) => { try { const result = await createUser({ ...req.body, roleName: "Viewer" }); res.cookie(AUTH_COOKIE, result.token, authCookieOptions).status(201).json({ user: result.user }); } catch (error) { next(error); } };
