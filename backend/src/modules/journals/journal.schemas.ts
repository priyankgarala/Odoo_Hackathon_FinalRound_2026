...
const line = z.object({
  accountId: id,
  partnerId: id.optional().nullable(),
  debit: amount.default(0),
  credit: amount.default(0),
  description: z.string().trim().max(500).optional().transform((value) => value || null)
}).refine((value) => (value.debit > 0) !== (value.credit > 0), "Each line needs either a debit or a credit amount.");
...
export const createJournalSchema = z.object({
  body: z.object({
    ...
    defaultAccountId: id.optional().nullable()
  }),
  params: z.object({}),
  query: z.object({})
});
